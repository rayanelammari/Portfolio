...
function parcours_events()
{
	...
	function add_job(job, inverted)
	{
		...
		{
			document.querySelector('#parcours_section .parcours_content').innerHTML += `
	...
	{
		document.querySelector('#parcours_section .parcours_content').innerHTML = '';
		let inverted = data.parcours.length % 2 == 0;

		for (let job of data.parcours)
		{
			inverted = !inverted;
		...
		elements = document.querySelectorAll('#parcours_section .in_animation');
	...
	document.querySelectorAll('#parcours_section .sort_choices .choice').forEach((el) =>
	{
